...
import tk.azertyfun.dcputoolchain.emulator.GenericClock;
import tk.azertyfun.dcputoolchain.emulator.GenericKeyboard;
import tk.azertyfun.dcputoolchain.emulator.LEM1802;
import tk.azertyfun.dcputoolchain.emulator.M35FD;
import tk.azertyfun.dcputoolchain.emulator.M525HD;


public class DebuggerInterface extends JFrame {
	private static final long	serialVersionUID	= -1717429947476584358L;

	private Action				goToAddressAction	= new AbstractAction("Go to address (Ctrl+G)") {
														private static final long	serialVersionUID	= -1875274991558862368L;
	...
	public DebuggerInterface(DCPU dcpu, TickingThread tickingThread, CallbackStop callbackStop){
		...
		Action stepAction = new AbstractAction("Step (Ctrl+S)") {
			private static final long	serialVersionUID	= -3430397075238953871L;
		...
		Action runpauseAction = new AbstractAction("Run/Pause (Ctrl+R)") {
			private static final long	serialVersionUID	= -8857508135235991254L;
	...
	public void goToAddress(){
		// String message = JOptionPane.showInputDialog("Please input an address in RAM."); //Makes X kill the app on
		// dispose(), for some reason. Google tells me to change the parent, which doesn't help. Oh, well.
		...
		ActionListener actionListener = actionEvent -> {
			...
			if(input.equalsIgnoreCase("PC")){
				...
				try{
					...
					// A JOptionPane should display an error, but since they crash on me I'm too lazy to implement one
					// myself.
				}
			}
